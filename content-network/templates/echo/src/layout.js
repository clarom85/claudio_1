...
export function renderHomePage(articles,site){
  ...
  // Mid-page CTA banner replaces an unfilled adUnit that was leaving a
  // ~90px blank between the hero and Latest Stories. For medicarepriceguide
  // we use the ParentCare Finder funnel; other echo-themed sites fall back
  // to the original ad slot.
  const isMpg = (site.url || '').includes('medicarepriceguide.com');
  const ctaBanner = `<aside style="background:linear-gradient(135deg,#faf6f1 0%,#f5ecdf 100%);border:1px solid #e6dccf;border-left:5px solid #c4622d;border-radius:10px;padding:30px 34px;margin:28px 0 32px;display:grid;grid-template-columns:1fr auto;gap:28px;align-items:center">
    <div>
      <p style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#c4622d;font-weight:700;margin:0 0 10px">Free 2-minute care assessment</p>
      <h2 style="font-family:'Cormorant Garamond',Georgia,serif;font-weight:500;font-size:30px;line-height:1.15;color:#3d2b1f;margin:0 0 8px">Worried about an aging parent?</h2>
      <p style="font-size:15px;color:#7a6a5a;line-height:1.65;margin:0;max-width:520px">Take a private 2-minute assessment and compare home care, assisted living, or memory care options matched to your loved one's needs, location, and budget.</p>
    </div>
    <a href="/find-care/" style="background:#c4622d;color:#fff;padding:15px 28px;border-radius:8px;font-weight:700;font-size:15px;text-decoration:none;letter-spacing:.3px;white-space:nowrap;justify-self:end;box-shadow:0 4px 14px rgba(196,98,45,.25)">Begin Assessment &rarr;</a>
  </aside>
  <style>@media(max-width:640px){.wrap aside[style*='Free 2-minute']{grid-template-columns:1fr;text-align:left}.wrap aside[style*='Free 2-minute']>a{justify-self:start}}</style>`;

  const middleSlot = isMpg ? ctaBanner : adUnit('leaderboard');
  const body=`${renderHeader(site)}<main class="site-main"><div class="wrap">${adUnit('leaderboard')}${h1Html}${heroHtml}${costTrackerHtml}${middleSlot}${gridHtml}</div></main>${renderFooter(site)}`;
