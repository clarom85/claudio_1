...
body{font-family:var(--ff-body);background:var(--light);color:var(--dark);line-height:1.7;overflow-x:hidden}
...
@media(max-width:640px){.hdr-nav{flex-wrap:wrap;justify-content:flex-end}.nav-toggle{display:flex}.hdr-nav ul{display:none;flex-direction:column;width:100%;order:2}.hdr-nav ul.nav-open{display:flex}.hdr-nav a{padding:14px 20px;border-bottom:1px solid rgba(255,255,255,.15)}.hdr-ad{display:none!important}}
...
/* Content */
.art-body{background:var(--white);padding:28px;border-radius:2px;box-shadow:var(--shadow);overflow-x:hidden;overflow-wrap:break-word;word-break:break-word}
...
/* Trust box */
.trust-box{background:#f3f9f5;border:1px solid #c5ddd0;border-left:5px solid var(--green);padding:18px 22px;margin:0 0 24px;border-radius:2px}
.trust-box-hdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-wrap:wrap;gap:8px}
.trust-box-title{font-family:var(--ff-head);font-size:15px;font-weight:700;color:var(--green);display:flex;align-items:center;gap:6px}
...
.trust-box-reviewer{color:var(--green);font-weight:600}
...
<header>
  <div class="hdr-main">
    <div class="wrap">
      ...
      <div class="tagline">${site.headerBadge||'Trusted · Accurate · In-Depth'}</div>
      <div class="hdr-rule"></div>
    </div>
  </div>
  <nav class="hdr-nav"><button class="nav-toggle" id="nav-toggle" aria-label="Open menu" aria-expanded="false">&#9776;</button><ul id="main-nav"><li><a href="/">Home</a></li>${(site.categories||[]).map(c=>`<li><a href="/category/${c.slug}/">${esc(c.name)}</a></li>`).join('')}${site.toolSlug?`<li><a href="/tools/${site.toolSlug}/" style="color:#c9a84c;font-weight:700">${site.toolLabel||'Free Calculator'}</a></li>`:''}${site.hasCostTracker?`<li><a href="/cost-tracker/">Cost Tracker</a></li>`:''}</ul></nav>
</header>
...
setTimeout(function(){document.querySelectorAll('[data-type="_mgwidget"]').forEach(function(w){if(!w.innerHTML.trim()||w.offsetHeight<10){w.style.display='none';w.style.margin='0';}});},3000);
...
export function renderArticlePage(article,site,relatedArticles=[]){
  ...
  const updatedBadgeHtml=showUpdated?`<span style="color:#888;font-size:11px;margin-left:8px;">· Reviewed ${updatedStr}</span>`:'';
...
export function renderHomePage(articles,site){
  ...
  const heroSideCta=site.toolSlug
    ?`<div style="background:var(--green);padding:18px 20px;margin-top:4px;text-align:center"><p style="color:rgba(255,255,255,.85);font-size:13px;margin:0 0 12px;line-height:1.5">Get an instant cost estimate for your project.</p><a href="/tools/${site.toolSlug}/" style="display:block;background:#c9a84c;color:#1a1a2e;padding:10px 16px;font-weight:700;font-size:14px;text-decoration:none">${site.toolLabel||'Free Calculator'} →</a></div>`
